    ...
    internal class ScopedProcessingService : IScopedProcessingService
    {
        ...
        public async Task DoWork(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                foreach (var newItem in filteredItems)
                {
                    ...
                    Console.WriteLine(!sendtask.IsFaulted
                        ? $"Notification pushed for : {newItem.Id}"
                        : $"Error on notification pushed for : {newItem.Id} {sendtask.Exception.Message}");
                }
        ...
        private List<SyndicationItem> GetRssFeeds()
        {
            ...
            foreach (string feed in feeds)
            {
                ...
                {
                    Console.WriteLine($"Error to read Rss feed : {feed}  {e.InnerException}");
                }
            }

            finalItems.OrderBy(x => x.PublishDate);
